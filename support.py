...
def showImages(imgarray, indexes):
    ...
    for i in range(n):
        ...
        s = "Label: {}".format(int(float(imgarrayY[indexes[i]])))
        plt.title(s)
        plt.axis('off')
        plt.imshow(pixels, cmap='gray')
    plt.savefig("sample.png",fmt="png")
    plt.show()
...
def showPredictedLabels(imgarray, indexes, labels):
    ...
    for i in range(n):
        plt.subplot(w, l, i+1)
        pixels = np.array(imgarrayX[indexes[i]]*255).reshape((28, 28))
        s = "True: {}, Pred: {}".format(int(float(imgarrayY[indexes[i]])), int(float(labels[i])))
        plt.title(s)
        plt.axis('off')
        plt.imshow(pixels, cmap='gray')
    plt.savefig("predect.png",fmt="png")
    plt.show()
